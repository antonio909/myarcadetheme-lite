<?php
/**
 * Nome do Modelo: Blog Post
 */

get_header(); ?>

<div class="cont">
	<?php if ( myarcadetheme_get_option( 'single_post_breadcrumbs', 1 ) ) { myarcadetheme_breadcrumb(); } ?>
	...
	<div class="cntcls <?php echo 'sidebar_' . myarcadetheme_get_option( 'sidebar_position', 'right' ); ?>">
		<main class="main-cn cols-n9">
				...
				while ( have_posts() ) : 
					...
					<article>
						<section class="post-sngl">
							<header>
								...
								<p>
									...
									<a href="<?php echo get_author_posts_url( get_the_author_meta('ID') ); ?>" title="<?php printf( esc_attr__( 'Ver todas as postagens de %s', 'myarcadetheme' ), get_the_author() ); ?>">
										<span class="fa-user"><?php the_author(); ?></span>
										<span class="fa-calendar"><?php echo get_the_date('d'); ?> <?php echo get_the_date('M'); ?> , <?php echo get_the_date('Y'); ?></span>
										<span class="fa-comments"><?php echo number_format_i18n( get_comments_number() ); ?></span>
									</a>
								</p>
							</header>
							
							<div class="txcn">
								<?php if ( myarcadetheme_get_option( 'single_post_featured_image', 1 ) ) { the_post_thumbnail( 'large', array( 'class' => 'aligncenter' ) ); } ?>
								<?php the_content(); ?>
							</div>
							<?php
							// Exibir alguns links de gerenciamento se o usuário connectado for um administrador
						...
						<footer>
							...
							<div class="pst-shre">
								<strong><?php _e( 'Compartilhar:', 'myarcadetheme' ); ?></strong>
								...
								<a rel="nofollow" onclick="window.open ('https://www.twitter.com/share?url=<?php the_permalink(); ?>', 'Twitter', 'toolbar=0, status=0, width=650, height=450');" href="javascripti: void(0);" class="fa-twitter"><span><?php _e( 'Twitter', 'myarcadetheme' ); ?></span></a>
			...
			if ( myarcadetheme_get_option( 'single_post_comments', 1 ) ) { myarcadetheme_comments(); }
			?>
		</main>
		<?php get_sidebar(); ?>
	</div>
